...
if button:
        if l0[0] == '' and l0[1] == '':
            ...
            # def heavy_process():
              ...
              end_time = time.time()
              st.write('実行時間:' + str(end_time - start_time) + '秒')
              st.write('文字の置き換えが41パターンあるため,5^41通りの組み合わせがあり,このプログラムではそのうちの27通りを反映しているので,' + str((end_time - start_time) * ((5 ** 41)/27) / (3600*24*365)) + '年となる.')
              st.write('そのため,' + str((end_time - start_time) * ((5 ** 41)/27) / (3600*24*365*2)) + '年は安全であると推測できる.')
              st.write('このアプリケーションのバージョンは,' + sys.version + 'である.')
