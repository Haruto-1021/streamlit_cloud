...
if button:
        if l0[0] == '' and l0[1] == '':
            ...
            st.write('名字を入力してください')
              ...
              st.write('文字の置き換えが22パターンあるため、5^22通りの組み合わせがあり、このプログラムではそのうちの23通りを反映しているので、' + str((end_time - start_time) * ((5 ** 22)/23) / (3600*24)) + '日、すなわち、' + str((end_time - start_time) * ((5 ** 22)/23) / (3600*24*365)) + '年かかります。')
              st.write('そのため、' + str((end_time - start_time) * ((5 ** 22)/23) / ((3600*24) * 2)) + '日、すなわち、' + str((end_time - start_time) * ((5 ** 22)/23) / (3600*24*365*2)) + '年は安全です。')
