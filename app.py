...
bef0 = ['a', 'i', 'i', 'o', 's', 's', 'u', 'u', 'v', 'v', 'w', 'w', 'w', 'w', 'z', 'A', 'I', 'I', 'O', 'S', 'S', 'U', 'U', 'V', 'V', 'W', 'W', 'W', 'W', 'Z'] # 変化前の文字列
aft0 = ['@', '1', '!', '0', '5', '$', 'v', 'V', 'u', 'U', 'uu', 'UU', 'vv', 'VV', '2', '@', '1', '!', '0', '5', '$', 'v', 'V', 'u', 'U', 'uu', 'UU', 'vv', 'VV', '2'] # 変化後の文字列
...
if button:
        if l0[0] == '' and l0[1] == '':
            ...
            # def heavy_process():
              ...
              st.write('文字の置き換えが41パターンあるため、5^41通りの組み合わせがあり、このプログラムではそのうちの30通りを反映しているので、' + str((end_time - start_time) * ((5 ** 41)/30) / (3600*24)) + '日、すなわち、' + str((end_time - start_time) * ((5 ** 41)/30) / (3600*24*365)) + '年かかります。')
              st.write('そのため、' + str((end_time - start_time) * ((5 ** 41)/30) / ((3600*24) * 2)) + '日、すなわち、' + str((end_time - start_time) * ((5 ** 41)/30) / (3600*24*365*2)) + '年は安全であると推測できます。')
